...
def main():
    ...
    testset = torchvision.datasets.ImageFolder(root=path_real, transform=transform_test)
    model.eval()
    count = 0
    torch.tensor([1,2])
    ...
    for data, img_np in testset:
        ...
        for idx in indices[0][:5]:
            datals.append((classes[idx], percentage[idx].item()))
        ...
        plt.tight_layout()
        count = count + 1
        plt.savefig(path_file + "final{}.jpg".format(count), bbox_inches='tight', transparent=True)
